...
SELECT country, COUNT(*) AS count_suppliers FROM suppliers GROUP BY country ORDER BY count_suppliers DESC;
...
SELECT ship_country, SUM(freight) AS total FROM orders WHERE ship_region IS NOT NULL GROUP BY ship_country HAVING SUM(freight) > 2750 ORDER BY total DESC;
